...
void tridiag_neumann0(double alpha, double b, double gamma, long* n_cells, int size, gsl_vector *lower, gsl_vector *main, gsl_vector *upper) {
    // Fill main diagonal with 1 + 2 * alpha - gamma - b * n_cells
    double main_value = 1 + 2 * alpha - gamma;
    for (int i = 0; i < size; ++i) {
        gsl_vector_set(main, i, main_value - b * n_cells[i]);
    }

    // Fill lower diagonal: -alpha except the last element (-2 * alpha)
    for (int i = 0; i < size - 1; ++i) {
        gsl_vector_set(lower, i, (i == size - 2) ? -2 * alpha : -alpha);
    }

    // Fill upper diagonal: -alpha, except the first element (-2 * alpha)
    gsl_vector_set(upper, 0, -2 * alpha);
    for (int i = 1; i < size - 1; ++i) {
        gsl_vector_set(upper, i, -alpha);
    }
}

void cn_neumann0(gsl_matrix* A, int size) {
    // Set Crank-Nicolson matrix with Neumann BC = 0
    for (int i = 0; i < size; ++i) {
        if (i == 0) {
            gsl_matrix_set(A, i, i, -2); // First diagonal
            if (i < size - 1) {
                gsl_matrix_set(A, i, i + 1, 2); // First super-diagonal
            }
        } else if (i == size - 1) {
            gsl_matrix_set(A, i, i - 1, 2); // Last sub-diagonal
            gsl_matrix_set(A, i, i, -2); // Last diagonal
        } else {
            gsl_matrix_set(A, i, i - 1, 1); // Sub-diagonal
            gsl_matrix_set(A, i, i, -2); // Main diagonal
            gsl_matrix_set(A, i, i + 1, 1); // Super-diagonal
        }
    }
}
...
void EvalPopulation(struct sim_parameters *params,
    ...
    lambda[0] = eigenvalue; 
    kpop[0] = density_previous[0]*eigenvalue + diff_coef_pop*(density_previous[1]-density_previous[0]); 
    ...
    kpop[n_xslots-1] = density_previous[n_xslots-1]*eigenvalue + diff_coef_pop*(density_previous[n_xslots-2]-density_previous[n_xslots-1]); 
...
void EvalOxygen(struct sim_parameters *params,
    ...
    koxy[0] = reaction0+diff_coef*diffusion0; 
    ...
    koxy[n_xslots-1] = reactionn+diff_coef*diffusionn; 
